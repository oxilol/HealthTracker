...
import { useCardioCalendar } from '../hooks/useCardioCalendar';
...
export function CardioCalendar({ selectedDate, onSelectDate, onClose }: CardioCalendarProps) {
  const today = new Date();
  const [viewMonth, setViewMonth] = useState(today.getMonth());
  const [viewYear, setViewYear] = useState(today.getFullYear());
  const { daySummaries, loading } = useCardioCalendar(viewMonth, viewYear);
    ...
    <div className="fixed inset-0 z-[60] bg-neutral-950/95 backdrop-blur-sm flex flex-col animate-fade-in">
      <div className="w-full max-w-md mx-auto flex flex-col h-full animate-slide-up">
        <div className="flex items-center gap-3 p-4 pt-6">
          ...
          <h2 className="text-lg font-semibold text-neutral-100">Cardio Calendar</h2>
          <button onClick={goToToday} className="ml-auto text-xs text-cyan-400 font-medium hover:text-cyan-300 transition-colors">
            Today
          </button>
        </div>
        ...
        <div className="px-4 flex-1">
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-7 gap-1">
              ...
              {days.map((day) => {
                ...
                const hasSession = daySummaries.has(dateStr);
          ...
          <div className="flex items-center justify-center gap-5 mt-5 pb-4">
            <div className="flex items-center gap-1.5">
              <div className="w-3 h-3 rounded bg-cyan-500/15 border border-cyan-500/30" />
              <span className="text-[11px] text-neutral-500">Cardio Done</span>
            </div>
            <div className="flex items-center gap-1.5">
